...
namespace PropnexPoster.Avalonia
{
    ...
    public class PosterRunInfo : INotifyPropertyChanged
    {
        ...
        // PosterRun.Run() 在后台线程里修改这些属性；Avalonia 的绑定引擎不会像 WPF 那样
        // 自动把非 UI 线程发出的 PropertyChanged 封送到 UI 线程，这里手动转发一次。
    ...
    public class PosterRun : Volo.Abp.DependencyInjection.ITransientDependency
    {
        ...
        public async Task Run()
        {
            ...
            try
            {
                _logger = new LoggerConfiguration()
                  ...
                  .CreateLogger();
                var proxyIp = "";
                if (AvaloniaModule.AppConfiguration.IsProxy)
                {
                    proxyIp = AvaloniaModule.AppConfiguration.GetProxy();
                }
        ...
        private async Task<bool> UploadAllMediaAsync(GuruTask task, GuruTaskListing listing, Api _api)
        {
            ...
            async Task<bool> uploadStep(Func<GuruTaskListing, Api, Task<HttpResult<string>>> upload, string errorMessage)
            {
                ...
                await SlackBotMessage.SendAsync($"TaskId:{task.Id}-TaskItemid:{listing.TaskItemId}-ListingId{listing.Listing.Id} {errorMessage}  {AvaloniaModule.AppConfiguration.MachineNumber} <@U01DQLBLWNL>");
                return false;
            }
        }
