    ...
    public class PosterRun : Volo.Abp.DependencyInjection.ITransientDependency
    {
        ...
        public async Task Run()
        {
            ...
            try
            {
                ...
                {
                    proxyIp = ConsoleModule.AppConfiguration.GetProxy();
                }
                            ...
                            foreach (var listing in task.Listings.Listings)
                            {
                                ...
                                else
                                {
                                    if (result.Message.Contains("Postal code is already being used"))
                                    {
                                        ...
                                        {
                                            await SlackBotMessage.SendAsync($"{task.Id}-{listing.TaskItemId}-{listing.Listing.Id} {result.Message}  {ConsoleModule.AppConfiguration.MachineNumber}  <@U01DQLBLWNL>");
